...
const BoardArrange = (props) => {
    ...
    const response = [[2,1,1],[0,1,1],[1,0,1]];
     //const response2 =elementsRotting();
          ...
          return (
               <div className='row'>
                    {rowLineState.map((i, col) => (
                        // <Cell key={col} classVal={positionsState[rowval][col] == 2 ? classColor : 'beige'} showQueen={false} />
                        <SelectionBar row={rowval} col={col} key={col+rowval} />
                    ))}
               </div>
          )
     }
     ...
     return (
          <Container fluid className='initialboard col'>
                <br />
               <button type="button" class="btn btn-comic">Show changes after each move</button>
               <br />
